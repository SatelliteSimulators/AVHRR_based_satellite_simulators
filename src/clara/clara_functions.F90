...
SUBROUTINE CTTH(d1,ins,CDR,model,S,inter)
   ...
   IF (inter%cflag(ins) .EQ. 3) THEN
      ! OPAQUE CLOUDS
      CALL CLARA_CTTH_OPAQUE(d1,ins,CDR,model,S,inter)
   ...
   !PRINT '(A10,1x,I1,1x,2(A10,1x,f7.1))', "cflag=",inter%cflag(ins),"cth_model=",cth,"cth=",inter%cth(ins)

END SUBROUTINE CTTH

SUBROUTINE CLARA_CTTH_OPAQUE(d1, ins, CDR, model, S, inter)
   INTEGER, INTENT(in)           :: d1,ins
   CHARACTER(*),INTENT(in)       :: CDR
   TYPE(subset), INTENT(in)      :: S
   TYPE(model_type), INTENT(in)  :: model
   ...
   IF (CDR .EQ. 'clara-a2') THEN
      CALL CLARA_A2_OPAQUE(d1,ins,model,S,inter)
   ELSEIF (CDR .EQ. 'clara-a3') THEN
      CALL CLARA_A3_OPAQUE(d1,ins,model,S,inter)
   ENDIF
...
SUBROUTINE CLARA_A3_OPAQUE(d1,ins,model,S,inter)
   ...
   DEALLOCATE(PPS_offset)
END SUBROUTINE CLARA_A3_OPAQUE

SUBROUTINE CLARA_A2_OPAQUE(d1,ins,model,S,inter)
   !
   ...
   ELSE
      ...
      inter%ctt(ins) = ctt
   END IF
END SUBROUTINE CLARA_A2_OPAQUE
  ...
  SUBROUTINE DAY_ADD(IN,S,ngrids,n_pbins,n_tbins)
    ...
    ! reset these values
    CALL INITIALISE_CLARA_SIM(IN%av,ngrids,-9._wp,n_pbins,n_tbins)
