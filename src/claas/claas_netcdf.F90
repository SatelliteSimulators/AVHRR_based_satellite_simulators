...
CONTAINS
  ...
  SUBROUTINE MAKE_NETCDF(model,sim,O,iday,S,CLAAS)
    ...
    TYPE(CLAAS_type),                    INTENT(in) :: CLAAS
   ...
   END IF
   CALL addVariable(ncid,'POD_tau_bin_centers',O,A,podid ,CLAAS=CLAAS)
   CALL addVariable(ncid,'POD_tau_bin_edges',  O,A,podid2,CLAAS=CLAAS)
   ! END auxiliary
   ! --------------------------------
   ...
   ! SIMULATORS
   ! --------------------
    IF (V%cfc      ) CALL addVariable(ncid,'cfc'      ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cfc_day  ) CALL addVariable(ncid,'cfc_day'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cfc_low  ) CALL addVariable(ncid,'cfc_low'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cfc_mid  ) CALL addVariable(ncid,'cfc_mid'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cfc_high ) CALL addVariable(ncid,'cfc_high' ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cflag_tot) CALL addVariable(ncid,'cflag_tot',O,A,lnid,ltid,cldthickid,tid,CLAAS=CLAAS)
    IF (V%cot_ice  ) CALL addVariable(ncid,'cot_ice'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cot_liq  ) CALL addVariable(ncid,'cot_liq'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%cth      ) CALL addVariable(ncid,'cth'      ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%ctp      ) CALL addVariable(ncid,'ctp'      ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%ctp_log  ) CALL addVariable(ncid,'ctp_log'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%ctt      ) CALL addVariable(ncid,'ctt'      ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%iwp      ) CALL addVariable(ncid,'iwp'      ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%lwp      ) CALL addVariable(ncid,'lwp'      ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%ref_ice  ) CALL addVariable(ncid,'ref_ice'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%ref_liq  ) CALL addVariable(ncid,'ref_liq'  ,O,A,lnid,ltid,tid,CLAAS=CLAAS)
    IF (V%hist2d_cot_ctp) &
         CALL addVariable(ncid,'hist2d_cot_ctp',O,A,lnid,ltid,tauid,prsid,phaseid,tid,CLAAS=CLAAS)
    CALL addVariable(ncid,'POD_layers',O,A,lnid,ltid,podid,sunid,tid,CLAAS=CLAAS)
    CALL addVariable(ncid,'FAR',O,A,lnid,ltid,sunid,tid,CLAAS=CLAAS)
...
SUBROUTINE addVariable(ncid,variable,O,A,&
   ...
   TYPE(CLAAS_type),          OPTIONAL,  INTENT(in) :: CLAAS
   ...
   SELECT CASE (variable)
   CASE ('cfc')
      CALL putVar(ncid,varid,A,CLAAS%av%cfc)
   CASE ('cfc_day')
      CALL putVar(ncid,varid,A,CLAAS%av%cfc_day)
   CASE ('cfc_low')
      CALL putVar(ncid,varid,A,CLAAS%av%cfc_low)
   CASE ('cfc_mid')
      CALL putVar(ncid,varid,A,CLAAS%av%cfc_mid)
   CASE ('cfc_high')
      CALL putVar(ncid,varid,A,CLAAS%av%cfc_high)
   CASE ('cflag_tot')
      CALL putVar(ncid,varid,A,data2i=CLAAS%av%cflag_tot)
   CASE ('cot_ice')
      CALL putVar(ncid,varid,A,CLAAS%av%cot_ice)
   CASE ('cot_liq')
      CALL putVar(ncid,varid,A,CLAAS%av%cot_liq)
   CASE ('cth')
      CALL putVar(ncid,varid,A,CLAAS%av%cth)
   CASE ('ctp')
      CALL putVar(ncid,varid,A,CLAAS%av%ctp)
   CASE ('ctp_log')
      CALL putVar(ncid,varid,A,CLAAS%av%ctp_log)
   CASE ('ctt')
      CALL putVar(ncid,varid,A,CLAAS%av%ctt)
   CASE ('iwp')
      CALL putVar(ncid,varid,A,CLAAS%av%iwp)
   CASE ('lwp')
      CALL putVar(ncid,varid,A,CLAAS%av%lwp)
   CASE ('hist2d_cot_ctp')
      CALL putVar(ncid,varid,A,data4=CLAAS%av%hist2d_cot_ctp)
   CASE ('ref_liq')
      CALL putVar(ncid,varid,A,CLAAS%av%ref_liq)
   CASE ('ref_ice')
      CALL putVar(ncid,varid,A,CLAAS%av%ref_ice)
   CASE ('POD_layers')
      CALL putVar(ncid,varid,A,data3=O%sim_aux%POD_layers)
   CASE ('FAR')
      CALL putVar(ncid,varid,A,data2=O%sim_aux%FAR)
   END SELECT

  END SUBROUTINE addVariable

END MODULE CLAAS_netcdf
